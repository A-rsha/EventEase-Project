  ...
  return (
    <nav className="relative">
      ...
      <div className="flex justify-between items-center py-6 w-full fixed top-0 z-50 backdrop-blur-md px-6">
        ...
        <h1 className="text-4xl font-bold text-shadow-red-900 hover:text-black">
          EventEase
        </h1>

        <div className="hidden md:flex items-center gap-6 font-semibold">
          <Link className="no-underline! text-black" to="/">HOME</Link>
          <Link className="no-underline! text-black " to="/events">EVENTS</Link>
          <Link className="no-underline!  text-black " to="/about">ABOUT</Link>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <Link
            to="/events"
            className="no-underline! bg-black text-white px-3 py-2 rounded hover:bg-red-900 transition"
          >
            BUY TICKET
          </Link>
      ...
      {open && (
        <div className="md:hidden absolute top-20 left-0 w-full  backdrop-blur-md py-6 shadow-lg z-50">
          <div className="  flex flex-col items-center gap-6 font-semibold">
            <Link className="no-underline! text-black hover:text-red-800" onClick={() => setOpen(false)} to="/">HOME</Link>
            <Link className="no-underline! text-black hover:text-red-800" onClick={() => setOpen(false)} to="/events">EVENTS</Link>
            <Link className="no-underline! text-black hover:text-red-800" onClick={() => setOpen(false)} to="/about">ABOUT</Link>
          </div>
        </div>
      )}
     ...
     backdrop-blur-md border
        text-black shadow-2xl z-50 flex flex-col">
          ...
          <div className="px-6 pb-6 overflow-y-auto flex-1">
            ...
            <div className="grid grid-cols-3 gap-4 mt-8 text-center">
              <div className="bg-white/10 p-4 rounded-xl">
                ...
                <p className="text-xs text-black/70">Bookings</p>
              </div>

              <div className="bg-white/10 p-4 rounded-xl">
                <h3 className="text-2xl font-bold">{events.length}</h3>
                <p className="text-xs text-black/70">Events</p>
              </div>

              <div className="bg-white/10 p-4 rounded-xl">
                <h3 className="text-2xl font-bold">{payments.length}</h3>
                <p className="text-xs text-black/70">Payments</p>
              </div>
            </div>
                ...
                bookings.map((booking) => (
                  <div key={booking._id} className="bg-white/10 p-3 rounded-xl">
                    ...
                    {booking.bookingStatus === "confirmed" && (
                      ...
                      className="bg-black text-white px-3 py-1 rounded text-sm mt-2">
                       cancel Booking
                      </button>
                    )}
                  </div>
            ...
            <button
              ...
              className="w-full bg-black text-white p-3 rounded-xl mt-8 transition"
            >
              Logout
            </button>
